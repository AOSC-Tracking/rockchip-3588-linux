...
#define HDMI21_MAX_RATE			4800000000
...
static int rk_hdptx_post_enable_lane(struct rk_hdptx_phy *hdptx)
{
	...
	/* 3 lanes FRL mode */
	if (hdptx->hdmi_cfg.tmds_char_rate == FRL_6G3L_RATE ||
	    hdptx->hdmi_cfg.tmds_char_rate == FRL_3G3L_RATE)
...
static int rk_hdptx_frl_lcpll_cmn_config(struct rk_hdptx_phy *hdptx)
{
	...
	for (i = 0; i < ARRAY_SIZE(rk_hdptx_frl_lcpll_cfg); i++) {
		if (hdptx->hdmi_cfg.tmds_char_rate == rk_hdptx_frl_lcpll_cfg[i].rate) {
...
{
	if (hdptx->hdmi_cfg.tmds_char_rate <= HDMI20_MAX_RATE)
		return rk_hdptx_tmds_ropll_cmn_config(hdptx);

	if (hdptx->hdmi_cfg.tmds_char_rate == FRL_8G4L_RATE)
...
//TODO: drop callback & is_frl and rename tmds_char_rate to link_rate
static int rk_hdptx_phy_set_mode(struct phy *phy, enum phy_mode mode, int submode)
{
	struct rk_hdptx_phy *hdptx = phy_get_drvdata(phy);

	if (mode != PHY_MODE_HDMI && mode != PHY_MODE_DP)
		return -EINVAL;

	hdptx->is_frl = (mode == PHY_MODE_HDMI && submode == PHY_SUBMODE_FRL);
					   ...
					   struct phy_configure_opts_hdmi *hdmi)
{
	...
	if (hdptx->is_frl) {
		if (hdptx->hdmi_cfg.tmds_char_rate > HDMI21_MAX_RATE)
			return -EINVAL;

		if (hdptx->hdmi_cfg.tmds_char_rate != FRL_8G4L_RATE) {
			for (i = 0; i < ARRAY_SIZE(rk_hdptx_frl_lcpll_cfg); i++)
				if (hdmi->tmds_char_rate == rk_hdptx_frl_lcpll_cfg[i].rate)
					break;
			if (i == ARRAY_SIZE(rk_hdptx_frl_lcpll_cfg))
				return -EINVAL;
		}
	} else {
		if (hdmi->tmds_char_rate > HDMI20_MAX_RATE)
			return -EINVAL;
