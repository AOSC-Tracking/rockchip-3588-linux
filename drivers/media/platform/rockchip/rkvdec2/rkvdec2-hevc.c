...
// FIXME: This will include both tables (h264/h265) twice
...
struct rkvdec2_hevc_tile {
	u16 value0	: 12;
	u16 value1	: 12;
} __packed;

struct rkvdec2_sps_pps_packet {
	struct rkvdec2_hevc_sps sps;
	struct rkvdec2_hevc_pps pps;
	struct rkvdec2_hevc_tile column_width[10];
	struct rkvdec2_hevc_tile row_height[11];
	u32 zeroes[3];
	u32 zeroes_bits		: 6;
	u32 padding_bits	: 2;
	u32 padding;
...
struct rkvdec2_rps {
	struct rkvdec2_rps_refs refs[32];
	struct rkvdec2_rps_short_term_ref_set short_term_ref_sets[64];
} __attribute((__packed__));

struct rkvdec2_hevc_scaling_list {
...
static enum rkvdec2_image_fmt rkvdec2_hevc_get_image_fmt(struct rkvdec2_ctx *ctx,
...
static void compute_tiles_uniform(struct rkvdec2_hevc_run *run, u16 log2_min_cb_size,
...
static void compute_tiles_non_uniform(struct rkvdec2_hevc_run *run, u16 log2_min_cb_size,
				      u16 width, u16 height, s32 pic_in_cts_width, 
				      s32 pic_in_cts_height, u16 *column_width, u16 *row_height)
{
	const struct v4l2_ctrl_hevc_pps *pps = run->pps;
			    ...
			    struct rkvdec2_hevc_run *run)
{
	...
	hw_ps->sps.diff_max_min_luma_coding_block_size = sps->log2_diff_max_min_luma_coding_block_size;
	...
	hw_ps->sps.diff_max_min_transform_block_size = sps->log2_diff_max_min_luma_transform_block_size;
	...
	hw_ps->sps.scaling_list_enabled_flag = !!(sps->flags & V4L2_HEVC_SPS_FLAG_SCALING_LIST_ENABLED);
	hw_ps->sps.amp_enabled_flag = !!(sps->flags & V4L2_HEVC_SPS_FLAG_AMP_ENABLED);
	hw_ps->sps.sample_adaptive_offset_enabled_flag = !!(sps->flags & V4L2_HEVC_SPS_FLAG_SAMPLE_ADAPTIVE_OFFSET);

	pcm_enabled = !!(sps->flags & V4L2_HEVC_SPS_FLAG_PCM_ENABLED);
	hw_ps->sps.pcm_enabled_flag = pcm_enabled;
	hw_ps->sps.pcm_sample_bit_depth_luma = pcm_enabled ? sps->pcm_sample_bit_depth_luma_minus1 + 1 : 0;
	hw_ps->sps.pcm_sample_bit_depth_chroma = pcm_enabled ? sps->pcm_sample_bit_depth_chroma_minus1 + 1 : 0;
	hw_ps->sps.pcm_loop_filter_disabled_flag = !!(sps->flags & V4L2_HEVC_SPS_FLAG_PCM_LOOP_FILTER_DISABLED);
	hw_ps->sps.diff_max_min_pcm_luma_coding_block_size = sps->log2_diff_max_min_pcm_luma_coding_block_size;
	hw_ps->sps.min_pcm_luma_coding_block_size = pcm_enabled ? sps->log2_min_pcm_luma_coding_block_size_minus3 + 3 : 0;
	hw_ps->sps.num_short_term_ref_pic_sets = sps->num_short_term_ref_pic_sets;
	hw_ps->sps.long_term_ref_pics_present_flag = !!(sps->flags & V4L2_HEVC_SPS_FLAG_LONG_TERM_REF_PICS_PRESENT);
	hw_ps->sps.num_long_term_ref_pics_sps = sps->num_long_term_ref_pics_sps;
	hw_ps->sps.sps_temporal_mvp_enabled_flag = !!(sps->flags & V4L2_HEVC_SPS_FLAG_SPS_TEMPORAL_MVP_ENABLED);
	hw_ps->sps.strong_intra_smoothing_enabled_flag = !!(sps->flags & V4L2_HEVC_SPS_FLAG_STRONG_INTRA_SMOOTHING_ENABLED);
	...
	hw_ps->pps.dependent_slice_segments_enabled_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_DEPENDENT_SLICE_SEGMENT_ENABLED);
	hw_ps->pps.output_flag_present_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_OUTPUT_FLAG_PRESENT);
	hw_ps->pps.num_extra_slice_header_bits = pps->num_extra_slice_header_bits;
	hw_ps->pps.sign_data_hiding_enabled_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_SIGN_DATA_HIDING_ENABLED);
	hw_ps->pps.cabac_init_present_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_CABAC_INIT_PRESENT);
	...
	hw_ps->pps.constrained_intra_pred_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_CONSTRAINED_INTRA_PRED);
	hw_ps->pps.transform_skip_enabled_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_TRANSFORM_SKIP_ENABLED);
	hw_ps->pps.cu_qp_delta_enabled_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_CU_QP_DELTA_ENABLED);
	hw_ps->pps.log2_min_cb_size = log2_min_cb_size + sps->log2_diff_max_min_luma_coding_block_size - pps->diff_cu_qp_delta_depth;
	hw_ps->pps.pps_cb_qp_offset = pps->pps_cb_qp_offset;
	hw_ps->pps.pps_cr_qp_offset = pps->pps_cr_qp_offset;
	hw_ps->pps.pps_slice_chroma_qp_offsets_present_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_PPS_SLICE_CHROMA_QP_OFFSETS_PRESENT);
	...
	hw_ps->pps.transquant_bypass_enabled_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_TRANSQUANT_BYPASS_ENABLED);
	bool tiles_enabled = !!(pps->flags & V4L2_HEVC_PPS_FLAG_TILES_ENABLED);
	hw_ps->pps.tiles_enabled_flag = tiles_enabled;
	hw_ps->pps.entropy_coding_sync_enabled_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_ENTROPY_CODING_SYNC_ENABLED);
	hw_ps->pps.pps_loop_filter_across_slices_enabled_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_PPS_LOOP_FILTER_ACROSS_SLICES_ENABLED);
	hw_ps->pps.loop_filter_across_tiles_enabled_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_LOOP_FILTER_ACROSS_TILES_ENABLED);
	hw_ps->pps.deblocking_filter_override_enabled_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_DEBLOCKING_FILTER_OVERRIDE_ENABLED);
	hw_ps->pps.pps_deblocking_filter_disabled_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_PPS_DISABLE_DEBLOCKING_FILTER);
	hw_ps->pps.pps_beta_offset_div2 = pps->pps_beta_offset_div2;
	hw_ps->pps.pps_tc_offset_div2 = pps->pps_tc_offset_div2;
	hw_ps->pps.lists_modification_present_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_LISTS_MODIFICATION_PRESENT);
	hw_ps->pps.log2_parallel_merge_level = pps->log2_parallel_merge_level_minus2 + 2;
	hw_ps->pps.slice_segment_header_extension_present_flag = !!(pps->flags & V4L2_HEVC_PPS_FLAG_SLICE_SEGMENT_HEADER_EXTENSION_PRESENT);
...
static void set_ref_valid(struct rkvdec2_regs_hevc *regs, int id, u32 valid)
{
	switch(id) {
	case 0:
			     ...
			     struct rkvdec2_hevc_run *run)
{
	...
	struct rkvdec2_hevc_ctx *hevc_ctx = ctx->priv;
	struct rkvdec2_regs_hevc *regs = &hevc_ctx->regs;
	...
	/* Set HEVC mode */
	regs->common.reg009.dec_mode = RKVDEC2_MODE_HEVC;
	...
	/* Set hw pps address */
	offset = offsetof(struct rkvdec2_hevc_priv_tbl, param_set);
	regs->hevc_addr.pps_base = priv_start_addr + offset;

	/* Set hw rps address */
	offset = offsetof(struct rkvdec2_hevc_priv_tbl, rps);
	regs->hevc_addr.rps_base = priv_start_addr + offset;

	/* Set cabac table */
	offset = offsetof(struct rkvdec2_hevc_priv_tbl, cabac_table);
...
const struct rkvdec2_coded_fmt_ops rkvdec2_hevc_fmt_ops = {
	...
	.get_image_fmt = rkvdec2_hevc_get_image_fmt,
};
