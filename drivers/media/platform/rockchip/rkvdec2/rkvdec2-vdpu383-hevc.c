...
struct rkvdec2_hevc_sps_pps {
	...
	u16 max_transform_hierarchy_depth_intra		: 3;//69
	u16 scaling_list_enabled_flag			: 1;
	u16 amp_enabled_flag				: 1;
	u16 sample_adaptive_offset_enabled_flag		: 1; // is not set
	...
	u16 strong_intra_smoothing_enabled_flag		: 1;//104
	//TODO: SPS extensions in place of the reserved_0
	u16 reserved_0					: 7;
	u16 sps_max_dec_pic_buffering_minus1		: 4;
        u16 separate_colour_plane_flag			: 1;
        u16 high_precision_offsets_enabled_flag		: 1;
        u16 persistent_rice_adaptation_enabled_flag	: 1;//118
	...
	u16 num_ref_idx_l1_default_active		: 4; // 153
	...
	u16 transquant_bypass_enabled_flag		: 1;//180
	u16 tiles_enabled_flag				: 1;
	u16 entropy_coding_sync_enabled_flag		: 1;
	u16 pps_loop_filter_across_slices_enabled_flag	: 1; // is not set
	...
	u16 zeroes					: 3; //84(pps) + 118(sps) = 202

	// pps extensions
        u16 log2_max_transform_skip_block_size		: 2;
        u16 cross_component_prediction_enabled_flag	: 1;
        u16 chroma_qp_offset_list_enabled_flag		: 1; // 206 is not set
        u16 log2_min_cu_chroma_qp_delta_size		: 3; // 208 is not set
        ...
        u16 chroma_qp_offset_list_len_minus1		: 3; // 154(pps) + 118(sps) = 272 (aligned 34 bytes)
	...
	/* poc info */
	u16 reserved_poc				: 3; // 300
	//TODO: Check if all fields should be changed to u32
	u32 current_poc					: 32;
	u32 ref_pic_poc0				: 32;
	u32 ref_pic_poc1				: 32;
	u32 ref_pic_poc2				: 32;
	...
	u16 num_tile_rows				: 5;//720 + 118 = 838
	u32 column_width0				: 24;//862
	...
	u32 row_height8					: 24;
	u32 row_height9					: 24;
	u32 row_height10				: 24; // 1224 + 118 = 1342
	u32 padding_bits				: 2;
	u32 padding;
...
static void set_column_row(struct rkvdec2_hevc_sps_pps *hw_ps, u16 column, u16 row, int i)
{
	int shift = (i & 1) ? 12 : 0;
...
static void set_pps_ref_pic_poc(struct rkvdec2_hevc_sps_pps *hw_ps, u32 poc, int i)
{
	switch (i) {
	case 0:
			    ...
			    struct rkvdec2_hevc_run *run)
{
	...
	struct rkvdec2_hevc_sps_pps *hw_ps;
	s32 max_cu_width;
...
#if 0
static void set_ref_valid(struct vdpu383_regs_h26x *regs, int id, u32 valid)
{
	switch(id) {
	case 0:
...
static void rkvdec2_write_regs(struct rkvdec2_ctx *ctx)
{
	...
	struct rkvdec2_hevc_ctx *h265_ctx = ctx->priv;

	rkvdec2_memcpy_toio(rkvdec->regs + VDPU383_OFFSET_COMMON_REGS,
			    &h265_ctx->regs.common,
			    sizeof(h265_ctx->regs.common));
	dump_regs(ctx, (u32*)&h265_ctx->regs.common, sizeof(h265_ctx->regs.common)/4, VDPU383_OFFSET_COMMON_REGS);
	rkvdec2_memcpy_toio(rkvdec->regs + VDPU383_OFFSET_COMMON_ADDR_REGS,
			    &h265_ctx->regs.common_addr,
			    sizeof(h265_ctx->regs.common_addr));
	dump_regs(ctx, (u32*)&h265_ctx->regs.common_addr, sizeof(h265_ctx->regs.common_addr)/4, VDPU383_OFFSET_COMMON_ADDR_REGS);
	...
	dump_regs(ctx, (u32*)&h265_ctx->regs.h26x_params, sizeof(h265_ctx->regs.h26x_params)/4, VDPU383_OFFSET_CODEC_PARAMS_REGS);
	rkvdec2_memcpy_toio(rkvdec->regs + VDPU383_OFFSET_CODEC_ADDR_REGS,
			    &h265_ctx->regs.h26x_addr,
			    sizeof(h265_ctx->regs.h26x_addr));
	dump_regs(ctx, (u32*)&h265_ctx->regs.h26x_addr, sizeof(h265_ctx->regs.h26x_addr)/4, VDPU383_OFFSET_CODEC_ADDR_REGS);
			     ...
			     struct rkvdec2_hevc_run *run)
{
	...
	/* Set H264 mode */
	regs->common.reg8_dec_mode = RKVDEC2_MODE_HEVC;
	...
	regs->h26x_params.reg67_global_len = sizeof(struct rkvdec2_hevc_sps_pps) / 16;
...
static int rkvdec2_hevc_run(struct rkvdec2_ctx *ctx)
{
	...
	writel(0x01000000, rkvdec->link + 0x58);
...
const struct rkvdec2_coded_fmt_ops rkvdec2_vdpu383_hevc_fmt_ops = {
