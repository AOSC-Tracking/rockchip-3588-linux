...
struct snps_hdmirx_dev {
	...
	struct gpio_desc *hdmirx_5v_detection_gpio;
...
static bool tx_5v_power_present(struct snps_hdmirx_dev *hdmirx_dev)
{
	...
	for (i = 0; i < 10; i++) {
		...
		val = gpiod_get_value(hdmirx_dev->hdmirx_5v_detection_gpio);
		if (val > 0)
			cnt++;
				     ...
				     u32 phy_reg, u32 val)
{
	...
	if (!wait_for_completion_timeout(&hdmirx_dev->cr_write_done, msecs_to_jiffies(20))) {
...
static void hdmirx_controller_init(struct snps_hdmirx_dev *hdmirx_dev)
{
	...
	if (!wait_for_completion_timeout(&hdmirx_dev->timer_base_lock, msecs_to_jiffies(20)))
		dev_err(dev, "%s wait cr read done failed!\n", __func__);
...
static int hdmirx_wait_lock_and_get_timing(struct snps_hdmirx_dev *hdmirx_dev)
{
	...
	if (!wait_for_completion_timeout(&hdmirx_dev->avi_pkt_rcv, msecs_to_jiffies(300))) {
...
static int hdmirx_parse_dt(struct snps_hdmirx_dev *hdmirx_dev)
{
	...
	hdmirx_dev->hdmirx_5v_detection_gpio = devm_gpiod_get_optional(dev, "hdmirx-5v-detection",
								       GPIOD_IN);
	if (IS_ERR(hdmirx_dev->hdmirx_5v_detection_gpio)) {
		dev_err(dev, "failed to get hdmirx 5v detection gpio\n");
		return PTR_ERR(hdmirx_dev->hdmirx_5v_detection_gpio);
	}
	...
	hdmirx_dev->vo1_grf = syscon_regmap_lookup_by_phandle(np, "rockchip,vo1_grf");
...
static int hdmirx_power_on(struct snps_hdmirx_dev *hdmirx_dev)
{
...
static int hdmirx_probe(struct platform_device *pdev)
{
	...
	hdmirx_dev->regs = devm_platform_ioremap_resource_byname(pdev, "hdmirx_regs");
	...
	irq = gpiod_to_irq(hdmirx_dev->hdmirx_5v_detection_gpio);
	if (irq < 0) {
		dev_err(dev, "failed to get hdmirx 5v detection gpio irq\n");
		ret = irq;
		goto err_pm;
	}
	...
	if (ret) {
		dev_err(dev,
			"request hdmirx 5v detection gpio irq thread failed, ret:%d\n",
			ret);
		goto err_pm;
	}
