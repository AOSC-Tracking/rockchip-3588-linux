...
static void dw_hdmi_qp_rockchip_encoder_enable(struct drm_encoder *encoder)
{
	...
	dev_dbg(hdmi->dev, "%s port=%d frl_enabled=%d frl_rate_forced=%u frl_rate=%u\n", __func__,
		hdmi->port_id,
...
static int dw_hdmi_qp_rockchip_phy_configure(struct rockchip_hdmi_qp *hdmi,
					     ...
					     unsigned int bpc)
{
	...
	//TODO: drop submodes and add lanes & rate_per_lane in HDMI config API
	if (new_link_cfg->frl_enabled) {
		phy_cfg.hdmi.tmds_char_rate = new_link_cfg->frl_lanes *
					      new_link_cfg->frl_rate_per_lane * 100000000ULL;
		phy_set_mode_ext(hdmi->phy, PHY_MODE_HDMI, PHY_SUBMODE_FRL);
	} else {
		phy_cfg.hdmi.tmds_char_rate = tmds_char_rate;
		phy_set_mode_ext(hdmi->phy, PHY_MODE_HDMI, PHY_SUBMODE_TMDS);
	}
	...
	if (!ret) {
		*link_cfg = *new_link_cfg;
		...
		dev_dbg(hdmi->dev, "%s tmds=%llu frl_enabled=%d frl_rate_forced=%u frl_rate_per_lane=%u frl_lanes=%u\n", __func__,
			hdmi->tmds_char_rate,
			link_cfg->frl_enabled, link_cfg->frl_rate_forced,
			link_cfg->frl_rate_per_lane, link_cfg->frl_lanes);
	} else {
					 ...
					 struct drm_connector_state *conn_state)
{
	...
	if (conn_state->hdmi.tmds_char_rate > HDMI20_MAX_TMDSRATE) {
		...
		} else if (link_cfg.frl_rate_per_lane > 12 || !link_cfg.frl_rate_per_lane) {
	...
	return dw_hdmi_qp_rockchip_phy_configure(hdmi, &link_cfg,
					     ...
					     void *data, u8 frl_rate)
{
	...
	return dw_hdmi_qp_rockchip_phy_configure(hdmi, &link_cfg, 0, s->output_bpc);
}
